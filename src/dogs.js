...
function Dogs() {
  ...
  const [currentSrc, updateSrc] = useState("image/segments.gif");
  ...
  const picGet = async (theBreed) => {
    await axios
      .get(specificRequest.replace("**", theBreed))
      .then((response) => {
  ...
  const setUpOpeningBreed = (breeds, openingBreed) => {
    const firstFound = breeds.find(
      (item) =>
        item.text.toLowerCase().localeCompare(openingBreed.toLowerCase()) === 0
    );
    return firstFound !== undefined
      ? {
          full: firstFound.text,
          url: firstFound.text.toLowerCase().replace(" ", "/")
        }
      : {
          full: breeds[0].text,
          url: breeds[0].text.toLowerCase().replace(" ", "/")
        };
  };
  ...
  useEffect(() => {
    ...
    const full = async () => {
      ...
      await axios
        .get(specificRequest.replace("**", actualOpeningBreed.url))
        .then((response) => {
  ...
  return (
    <React.Fragment>
      ...
      <Button
        ...
        onClick={(event, data) => state(event, data, ANOTHER_PIC)}
      >
        Another Picture
      </Button>
      <Segment>
